...
const mockResults: CorrelationResult[] = [
  { pairA: "Kp Index", pairB: "Solar Wind Speed", correlation: 0.78, lag: "+3h", significance: "high" },
  { pairA: "Dst Index", pairB: "Kp Index", correlation: -0.65, lag: "-1h", significance: "medium" },
  { pairA: "IMF Bz", pairB: "Auroral Power", correlation: 0.54, lag: "+2h", significance: "medium" },
  { pairA: "Solar Wind", pairB: "Dst Index", correlation: -0.42, lag: "+6h", significance: "low" },
];

const getSignificanceColor = (sig: string) => {
  switch (sig) {
    case "high": return "bg-coherence-high/10 text-coherence-high border-coherence-high/15";
    case "medium": return "bg-coherence-medium/10 text-coherence-medium border-coherence-medium/15";
    default: return "bg-muted/10 text-muted-foreground/50 border-border/15";
  }
};
            ...
            {mockResults.map((result, idx) => (
              <div key={idx} className="px-3 py-2.5 rounded-lg bg-muted/5 border border-border/10">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <Badge variant="outline" className={`text-[9px] px-1.5 py-0 h-4 ${getSignificanceColor(result.significance)}`}>
                    {result.correlation.toFixed(2)}
                  </Badge>
                </div>
                <div className="flex items-center justify-between">
                  <div className="h-[3px] rounded-full bg-border/10 overflow-hidden flex-1 mr-3">
                    <div
                      className="h-full rounded-full transition-all duration-700"
                      style={{
                        width: `${Math.abs(result.correlation) * 100}%`,
                        background: result.correlation > 0
                          ? `linear-gradient(90deg, ${sphere.color}40, ${sphere.color}cc)`
                          : `linear-gradient(90deg, hsl(var(--coherence-low) / 0.4), hsl(var(--coherence-low) / 0.8))`,
                      }}
                    />
                  </div>
